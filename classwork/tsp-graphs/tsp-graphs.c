...
gcc tsp-graphs.c -o test && ./test && rm -rf ./test
*/
...
void matrixImplementation(City cities[], int totalCities) {
  ...
  printf("distance between cities 3 and 2 after edit: %.2f\n", getDistanceBetweenCitiesMatrix(graph, 3, 2));
  printf("distance between cities 2 and 3 after edit: %.2f\n", getDistanceBetweenCitiesMatrix(graph, 2, 3));

  removeCityAndAllEdgesMatrix(graph, 1);

  printf("distance between cities 1 and 2 after remove: %.2f\n", getDistanceBetweenCitiesMatrix(graph, 1, 2));
  printf("distance between cities 1 and 3 after remove: %.2f\n", getDistanceBetweenCitiesMatrix(graph, 1, 3));

  printf("distance between cities 3 and 999 (doesn't exist): %.2f\n", getDistanceBetweenCitiesMatrix(graph, 3, 999));
...
void adjacencyListImplementation(City cities[], int totalCities) {
  ...
  printf("distance between cities 3 and 999 (doesn't exist): %.2f\n", getDistanceBetweenCitiesListGraph(graph, 3, 999));
...
void readTSPFileAndRunImplementations(const char* filename) {
  ...
  // run BOTH implementations separately
  matrixImplementation(cities, dimension);
  adjacencyListImplementation(cities, dimension);

  free(cities);
}

int main() {
  readTSPFileAndRunImplementations("qa194.tsp");
